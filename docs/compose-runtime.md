...
- A prior release can deploy its `original` configuration revision or the application's `current` revision. The actual revision used is stored on the deployment independently from the release's original revision. Local sources remain deployable but do not claim immutable release provenance.
