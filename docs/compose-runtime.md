...
- Project-scoped named volumes and networks, workspace-local builds and binds, and loopback-only published ports are allowed. A custom volume or network name escapes the fixed application project namespace and requires an exact stored approval. Host-level namespaces, privileged services, devices and GPUs, added capabilities, unconfined security, Docker socket access, external binds, and non-loopback or unspecified ports also require an exact stored approval when supported. Escapes, remote resources, unverifiable effective resource names, and unsafe or unknown shapes are rejected.
