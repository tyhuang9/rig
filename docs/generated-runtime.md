...
The builder prepares writable application storage for the non-root build user and explicit read-only parent directories for Rig's component selectors and command mounts. Failure to read either a selector or command aborts the build; Rig must never silently run in the repository root or treat an empty substituted command as success. These build-time preparations do not make the runtime filesystem writable.
...
Pull requests run `.github/workflows/generated-runtime-lifecycle-ci.yml` on a disposable Ubuntu Docker host. The live compiler test exercises the production Compiler, BuilderManager, and protected temporary storage with an npm fixture whose lockfile is at the repository root and whose separate component directory contains a space. It verifies the accepted build directory, non-root command execution, the built artifact, image metadata/history redaction, and cleanup. Its release, plan, and artifact adapters are in-memory test fixtures; database persistence and source materialization have separate tests. This fixture has no external package dependencies and does not establish live builds for every supported framework or package manager.

The live lifecycle test uses fixture images with the production runtime engine and ingress manager. It routes two isolated applications with identical component aliases, keeps blue serving through an unhealthy candidate, switches to healthy green, completes an already-running blue request across that switch, and then removes blue. A deterministic executor unit test separately proves that old-slot removal cannot occur until the coordinator's drain wait completes. Hosted checks also require complete Docker cleanup and run the generated-runtime packages with Go's race detector in an independent job. Both jobs must pass. A checkout without a Docker daemon can compile and skip the opt-in live tests, but cannot substitute for a passing hosted job.
