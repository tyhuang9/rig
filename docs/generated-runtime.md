...
Pull requests run `.github/workflows/generated-runtime-lifecycle-ci.yml` on a disposable Ubuntu Docker host. The live compiler test exercises the production Compiler, BuilderManager, and protected temporary storage with an npm workspace, separate install/build roots, and a component directory containing a space. It verifies non-root command execution, the built artifact, image metadata/history redaction, and cleanup. Its release, plan, and artifact adapters are in-memory test fixtures; database persistence and source materialization have separate tests.

The live lifecycle test uses fixture images with the production runtime engine and ingress manager. It routes two isolated applications with identical component aliases, keeps blue serving through an unhealthy candidate, switches to healthy green, completes an already-running blue request across that switch, and then removes blue. A deterministic executor unit test separately proves that old-slot removal cannot occur until the coordinator's drain wait completes. Hosted checks also require complete Docker cleanup and run the generated-runtime packages with Go's race detector. A checkout without a Docker daemon can compile and skip the opt-in live tests, but cannot substitute for a passing hosted job.
