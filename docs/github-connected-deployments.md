...
Rig can deploy a selected GitHub.com repository without a user-managed checkout or installed Git CLI. The controller resolves a tracked branch to one commit, downloads a bounded snapshot, and installs it as an immutable managed release. Compose applications apply their selected Compose file on the controller machine; supported generated JavaScript and TypeScript applications use the separate generated runtime after editable setup review. Existing local-path applications remain supported. See [Generated JavaScript runtime operations](generated-runtime.md) for the generated strategy.
...
- Sources are either `local` or `github`. A GitHub source binds a connection, installation ID, immutable repository ID, display owner/name, and tracked branch. A Compose deployment additionally records its normalized repository-relative Compose path; a generated deployment uses its accepted setup and immutable plan revision.
...
- Docker Compose execution is disabled unless `hostd` starts with `--compose-runtime`; the separate generated executor requires `--generated-runtime`. `--fake-runtime` remains isolated development behavior and cannot be combined with either real runtime. Each real-runtime flag enables only its own execution strategy; neither configures GitHub or the relay.
...
3. Add `--compose-runtime` only when this controller is authorized to execute its selected application's Docker Compose workload through its local Docker endpoint. It is required to execute a manual Compose deployment, but it neither enables GitHub connection nor automatic deployment.
...
6. In **Add application**, choose **GitHub repository**, search the single repository picker, then select the branch. For Compose applications, also select the discovered Compose file. The picker aggregates accessible personal and organization installations and retains the connection, installation, and immutable repository IDs internally.
7. For a Compose application, create the application, add visible/secret configuration as needed, and use **Deploy latest**. The resulting release records the resolved commit SHA, archive hash, Compose path, managed workspace state, and configuration revision without credentials.

For a generated application, enable `--generated-runtime`, choose **Analyze project** after selecting the source, and review then accept its editable build and run setup. This is a separate execution strategy: it does not require `--compose-runtime` unless the same controller also serves Compose applications. The [generated-runtime runbook](generated-runtime.md) defines the supported source model, controller-local Docker and Caddy boundaries, approval flow, and recovery behavior.

For a GitHub-connected controller that can execute manual Compose deployments, only the Compose execution flag is required:
...
Repository and hosted workflows provide deterministic validation, persistence, protocol, UI, and failure-path evidence. Hosted coverage includes Chromium execution of the embedded-hostd and GitHub source-wizard flows, Windows controller coverage, a real Linux relay Docker Compose lifecycle, Linux race tests, real-filesystem permission/no-follow tests, a native linux/amd64 relay image, and PostgreSQL integration tests; the result of each hosted run remains visible in CI. Recorded external checks cover live device authorization, reload/resume and restart persistence, repository-access installation, private-repository selection, and branch discovery. Positive controller-application GitHub-to-Docker Compose execution and disconnect cleanup remain external promotion checks, alongside PostgreSQL restore, TLS/proxy, relay recovery/load, container hardening, multi-architecture images, SBOM/signature/provenance, additional browsers and physical devices, and assistive-technology checks listed in [TASKS.md](../TASKS.md#external-promotion-gates).
