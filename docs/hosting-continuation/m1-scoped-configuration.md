...
**Recorded:** 2026-09-24. **Base:** M0 local commit
...
not been merged or deployed. The M1 hosted Docker exit gate passed on the
candidate branch; full controller and browser deployment remains M2 work.
...
| Opt-in `TestLiveHostingNotesFixtureImages` and generated-runtime CI step | The first hosted run on `14cb408a2d8ce0c2ed6633648ba2752146c942d6` passed the frontend image and existing blue-green lifecycle but failed the API image build: BuildKit reported `cd: can't cd to /workspace/app`. The `32e08ad76c6a3ddf236d4117aead851ac14264d1` diagnostic run proved that the staged path was `api` but a separate BuildKit context probe copied `app`; both values had the same length and staged timestamp. The content-derived timestamp fix passed [hosted generated-runtime job 107463081017](https://github.com/tyhuang9/rig/actions/runs/35945723400/job/107463081017) on `b65bf1ebd8b95226231d36879ff1072c533c5866`. The [job on `9844dc2`](https://github.com/tyhuang9/rig/actions/runs/36025970708/job/107722468786) passed separate API, `frontend-public-A`, and `frontend-public-B` image checks in 27.89 seconds. The two frontend builds changed only `VITE_BUILD_LABEL`, emitted and served their respective label over HTTP without the other, excluded the synthetic runtime-secret prefix and secret key names from static assets, and produced different image IDs. The same job passed blue-green lifecycle and exact owned-resource cleanup. |
| Opt-in `TestLiveHostingNotesDatabaseRoundtrip` | [Hosted Docker job 107722468786](https://github.com/tyhuang9/rig/actions/runs/36025970708/job/107722468786) on `9844dc2e3856f592bcdaa8d2c0a3e91ad52ab906` passed in 53.22 seconds. It created and read a note through generated ingress and an external application-owned PostgreSQL container with verified TLS, called the HTTPS dependency, kept the API on its app-private bridge, rejected a wrong-CA candidate without replacing the serving one, then switched to the same image with a new scoped revision and read the note again. It checked exact selected Docker environment entries without logging values, scanned the generated API build context, image metadata/history, and manifest-selected saved image layers for synthetic secrets, and passed owned-resource cleanup. Hosted runner was Ubuntu 24.04.5 with Docker CLI/Engine 28.0.4, API 1.48, Buildx 0.37.1, and Compose 2.38.2. Earlier runs exposed and resolved fixture certificate identity, Docker env-file quoting, and OCI archive layout assumptions; see the PR history. |
...
| --- | --- | --- |
| CFG-01, CFG-06, CFG-08 | Scoped export, compiler identity, API/UI validation, and build-input tests pass. Hosted Docker built the API and two frontend variants with only `VITE_BUILD_LABEL` changed, observed distinct image IDs and emitted labels, and fetched the served static index and JavaScript over HTTP. The synthetic runtime secret was absent from the generated API build context, image metadata/history, and all manifest-selected image layers; its key and unique prefix were absent from the emitted and served frontend assets. The exact selected API runtime environment was checked without printing values. | A real browser journey through the deployed frontend and controller remains an M2 gate. |
...
| CFG-09, CFG-10 | Literal-value, invalid-input, immutable storage, masking, admin/CSRF, and editor tests pass. Hosted Docker checked the byte-exact selected API environment and the protected temporary export was removed after use. | Recheck a full browser deployment through the controller in M2. |
...
Docker evidence; PR #70 remains draft. The M1 engine-level exit gate passed on
`9844dc2e3856f592bcdaa8d2c0a3e91ad52ab906` in the exact hosted job above.
That test uses direct engine calls and therefore has no durable deployment-job
identity; the continuous controller journey is M2. All six workflows passed on
earlier published head `46828f4`:
...
on the fix head; the earlier cause remains unconfirmed. On `9844dc2`, the
Documentation, Relay Compose, Playwright, and hosted Docker jobs passed; the
generated-runtime race, Windows controller, and GitHub deployment jobs were
still running when this record was written. Neither live Neon nor a full
browser-through-controller deployment has been exercised. The M1 engine-level
gate is accepted; the product-level golden path remains open in M2.
