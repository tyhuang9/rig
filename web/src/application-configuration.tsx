...
export function ApplicationConfigurationPanel({ appId }: { appId: string }) {
  ...
  return <section className="configuration-panel" aria-labelledby="configuration-title">
    ...
    <form onSubmit={save} noValidate>
      {error && <div className="error-summary" ref={errorSummary} tabIndex={-1} role="alert"><span>{error}</span>{conflict && <button type="button" className="button small" onClick={reload}>Reload latest configuration</button>}</div>}
      <div className="configuration-group">
        <div className="configuration-group-heading"><div><h3>Variables</h3><p>Replacing the configuration removes variables not listed here.</p></div><button type="button" className="button small" onClick={() => { const id = createID("variable"); setVariables((rows) => [...rows, { id, key: "", value: "" }]); markDirty(); setPendingFocus(`${id}-key`); }}>Add variable</button></div>
        {variables.length === 0 ? <p className="configuration-empty">No variables configured.</p> : <div className="configuration-rows">{variables.map((row) => <div className="configuration-row" key={row.id}>
          <div className="field"><label htmlFor={`${row.id}-key`}>Variable name</label><input id={`${row.id}-key`} value={row.key} onChange={(event) => updateVariable(row.id, "key", event.target.value)} autoComplete="off" /></div>
          <div className="field"><label htmlFor={`${row.id}-value`}>Value</label><input id={`${row.id}-value`} value={row.value} onChange={(event) => updateVariable(row.id, "value", event.target.value)} autoComplete="off" /></div>
          <button type="button" className="button small configuration-remove" aria-label={`Remove variable ${row.key || "row"}`} onClick={() => { setVariables((rows) => rows.filter((item) => item.id !== row.id)); markDirty(); }}>Remove</button>
        </div>)}</div>}
      </div>

      <div className="configuration-group">
        <div className="configuration-group-heading"><div><h3>Secrets</h3><p>Leave a stored secret blank to preserve it. Enter a value only to replace it.</p></div><button type="button" className="button small" onClick={() => { const id = createID("secret"); setSecrets((rows) => [...rows, { id, key: "", value: "", stored: false }]); markDirty(); setPendingFocus(`${id}-key`); }}>Add secret</button></div>
        {secrets.length === 0 ? <p className="configuration-empty">No secrets configured.</p> : <div className="configuration-rows">{secrets.map((row) => {
          ...
          return <div className={`configuration-row${isRemoved ? " removed" : ""}`} key={row.id}>
            <div className="field"><label htmlFor={`${row.id}-key`}>Secret name</label><input id={`${row.id}-key`} value={row.key} disabled={row.stored} onChange={(event) => updateSecret(row.id, "key", event.target.value)} autoComplete="off" /></div>
            <div className="field"><label htmlFor={`${row.id}-value`}>{row.stored ? "Replacement value" : "Secret value"}</label><input id={`${row.id}-value`} type="password" value={row.value} disabled={isRemoved} placeholder={row.stored ? "Stored — leave blank to preserve" : ""} aria-describedby={row.stored && !isRemoved ? `${row.id}-stored` : undefined} onChange={(event) => updateSecret(row.id, "value", event.target.value)} autoComplete="new-password" />{row.stored && !isRemoved && <span id={`${row.id}-stored`} className="stored-secret">Stored</span>}</div>
            {isRemoved ? <button type="button" className="button small configuration-remove" onClick={() => { setRemoved((keys) => { const next = new Set(keys); next.delete(row.key); return next; }); markDirty(); }}>Undo remove</button> : <button type="button" className="button small configuration-remove" aria-label={`Remove secret ${row.key || "row"}`} onClick={() => { if (row.stored) setRemoved((keys) => new Set(keys).add(row.key)); else setSecrets((rows) => rows.filter((item) => item.id !== row.id)); markDirty(); }}>Remove</button>}
          </div>;
        })}</div>}
      </div>
      <div className="configuration-footer"><span aria-live="polite" role="status">{message}</span><button className="button primary" disabled={mutation.isPending || !dirty}>{mutation.isPending ? "Saving…" : "Save configuration"}</button></div>
    </form>
  </section>;
}
