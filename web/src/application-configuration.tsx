          ...
          return <fieldset className={`configuration-row${isRemoved ? " removed" : ""}`} key={row.id} aria-describedby={describedBy(apiErrors.secrets ? "configuration-secrets-error" : undefined, apiErrors.remove ? "configuration-remove-error" : undefined)}>
            ...
            <div className="configuration-row-fields">
              ...
              <div className="configuration-row-actions">
                {row.value && !isRemoved && <button type="button" className="button small" disabled={busy} aria-pressed={revealed.has(row.id)} aria-label={`Reveal typed value for secret ${row.key || index + 1}`} onClick={() => { if (saving.current) return; setRevealed((ids) => { const next = new Set(ids); if (next.has(row.id)) next.delete(row.id); else next.add(row.id); return next; }); }}>{revealed.has(row.id) ? "Hide value" : "Show value"}</button>}
