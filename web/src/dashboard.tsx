  ...
  return <>
    ...
    <div className="wizard">
      ...
      <form onSubmit={handleSubmit((values) => create.mutate(values), invalid)} noValidate>
        ...
        <button type="button" className="button" onClick={() => api.inspect(getValues("sourcePath")).then((result) => setInspection(result.findings.length > 0 ? result.findings.map((finding) => finding.message).join(" ") : `Found ${result.services.length} service${result.services.length === 1 ? "" : "s"}.`)).catch((error) => setInspection(error.message))}>Check source</button>
