// Code generated by go run ./cmd/openapi-gen; DO NOT EDIT.

export const openAPISHA256 = "4525769f9b26a4efcf2d77efb4ec1b0d11c0ce04200f1907405d97d815d5f353";

export const operations = {
  "acceptApplicationDeploymentPlan": { method: "PUT", path: "/api/v1/apps/{appId}/deployment-plan" },
  "approveApplicationDeploymentPlanMigration": { method: "POST", path: "/api/v1/apps/{appId}/deployment-plan/migration-approval" },
  "bootstrap": { method: "POST", path: "/api/v1/auth/bootstrap" },
  "bootstrapStatus": { method: "GET", path: "/api/v1/auth/bootstrap/status" },
  "cancelJob": { method: "POST", path: "/api/v1/jobs/{jobId}/cancel" },
  "createApplication": { method: "POST", path: "/api/v1/apps" },
  "deployApplication": { method: "POST", path: "/api/v1/apps/{appId}/deployments" },
  "deployRelease": { method: "POST", path: "/api/v1/apps/{appId}/releases/{releaseId}/deployments" },
  "disconnectSourceConnection": { method: "DELETE", path: "/api/v1/source-connections/{connectionId}" },
  "doctor": { method: "GET", path: "/api/v1/system/doctor" },
  "getApplication": { method: "GET", path: "/api/v1/apps/{appId}" },
  "getApplicationAutoDeploy": { method: "GET", path: "/api/v1/apps/{appId}/auto-deploy" },
  "getApplicationConfiguration": { method: "GET", path: "/api/v1/apps/{appId}/configuration" },
  "getApplicationDeploymentPlan": { method: "GET", path: "/api/v1/apps/{appId}/deployment-plan" },
  "getDefaultSourceConnection": { method: "GET", path: "/api/v1/source-connections/default" },
  "getDeploymentJobByIdempotency": { method: "GET", path: "/api/v1/apps/{appId}/deployment-jobs/by-idempotency" },
  "getJob": { method: "GET", path: "/api/v1/jobs/{jobId}" },
  "getRelayStatus": { method: "GET", path: "/api/v1/relay/status" },
  "grantRuntimeApproval": { method: "POST", path: "/api/v1/apps/{appId}/runtime-approvals" },
  "inspectImport": { method: "POST", path: "/api/v1/apps/import/inspect" },
  "listApplications": { method: "GET", path: "/api/v1/apps" },
  "listDefaultGitHubRepositories": { method: "GET", path: "/api/v1/source-connections/default/github/repositories" },
  "listDeployments": { method: "GET", path: "/api/v1/apps/{appId}/deployments" },
  "listGitHubBranches": { method: "GET", path: "/api/v1/source-connections/{connectionId}/github/installations/{installationId}/repositories/{repositoryId}/branches" },
  "listGitHubInstallations": { method: "GET", path: "/api/v1/source-connections/{connectionId}/github/installations" },
  "listGitHubRepositories": { method: "GET", path: "/api/v1/source-connections/{connectionId}/github/installations/{installationId}/repositories" },
  "listJobEvents": { method: "GET", path: "/api/v1/jobs/{jobId}/events" },
  "listJobs": { method: "GET", path: "/api/v1/jobs" },
  "listMachines": { method: "GET", path: "/api/v1/machines" },
  "listReleases": { method: "GET", path: "/api/v1/apps/{appId}/releases" },
  "listRuntimeApprovals": { method: "GET", path: "/api/v1/apps/{appId}/runtime-approvals" },
  "listServices": { method: "GET", path: "/api/v1/apps/{appId}/services" },
  "listSourceConnections": { method: "GET", path: "/api/v1/source-connections" },
  "login": { method: "POST", path: "/api/v1/auth/sessions" },
  "logout": { method: "DELETE", path: "/api/v1/auth/sessions/current" },
  "me": { method: "GET", path: "/api/v1/auth/me" },
  "pollDefaultGitHubDeviceConnection": { method: "POST", path: "/api/v1/source-connections/{connectionId}/device/{authorizationId}/poll" },
  "pollGitHubDeviceConnection": { method: "POST", path: "/api/v1/source-connections/{connectionId}/device/poll" },
  "pollRelayEnrollment": { method: "POST", path: "/api/v1/relay/enrollments/{enrollmentId}/poll" },
  "refreshSourceConnection": { method: "POST", path: "/api/v1/source-connections/{connectionId}/refresh" },
  "removeRelayBinding": { method: "DELETE", path: "/api/v1/relay/bindings/{bindingId}" },
  "replaceApplicationConfiguration": { method: "PUT", path: "/api/v1/apps/{appId}/configuration" },
  "replaceScopedApplicationConfiguration": { method: "PUT", path: "/api/v1/apps/{appId}/scoped-configuration" },
  "restartApplication": { method: "POST", path: "/api/v1/apps/{appId}/restart" },
  "resumeApplicationAutoDeploy": { method: "POST", path: "/api/v1/apps/{appId}/auto-deploy/resume" },
  "resumeJob": { method: "POST", path: "/api/v1/jobs/{jobId}/resume" },
  "revokeRuntimeApproval": { method: "DELETE", path: "/api/v1/apps/{appId}/runtime-approvals/{approvalId}" },
  "rotateCSRF": { method: "GET", path: "/api/v1/auth/csrf" },
  "startApplication": { method: "POST", path: "/api/v1/apps/{appId}/start" },
  "startDefaultGitHubDeviceConnection": { method: "POST", path: "/api/v1/source-connections/default/github/device" },
  "startGitHubDeviceConnection": { method: "POST", path: "/api/v1/source-connections/github/device" },
  "startRelayEnrollment": { method: "POST", path: "/api/v1/relay/enrollments" },
  "startRelayKeyRotation": { method: "POST", path: "/api/v1/relay/key-rotations" },
  "stopApplication": { method: "POST", path: "/api/v1/apps/{appId}/stop" },
  "streamJobEvents": { method: "GET", path: "/api/v1/jobs/{jobId}/events/stream" },
  "streamLogs": { method: "GET", path: "/api/v1/apps/{appId}/logs/stream" },
  "systemStatus": { method: "GET", path: "/api/v1/system/status" },
  "updateApplicationAutoDeploy": { method: "PUT", path: "/api/v1/apps/{appId}/auto-deploy" },
} as const;

export type OperationID = keyof typeof operations;

export type AcceptDeploymentPlanRequest = {
  "candidateId": string;
  "components"?: DeploymentPlanComponentInput[];
  "expectedCandidateDigest": string;
  "expectedRevisionNumber": number;
  "expectedSourceStructuralFingerprint": string;
  "installBehavior"?: string;
  "migrationCommand"?: string;
  "migrationEnvironmentKeys"?: string[];
  "packageManager"?: string;
  "setup"?: DeploymentSetupInput;
};

export type AnalysisAdvancedInput = {
  "componentId": string;
  "field": string;
  "reason": string;
  "required": boolean;
};

export type AnalysisCommand = {
  "command"?: string;
  "confidence"?: string;
  "environmentKeys"?: string[];
  "evidence": AnalysisEvidence[];
  "origin"?: string;
  "phase"?: string;
  "present": boolean;
  "provenance"?: string;
  "workingDirectory"?: string;
};

export type AnalysisComponent = {
  "build"?: AnalysisCommand;
  "evidence": AnalysisEvidence[];
  "findings": AnalysisFinding[];
  "framework": string;
  "healthProbe"?: AnalysisHealthProbe;
  "id": string;
  "internalPort"?: AnalysisValue;
  "kind": string;
  "migration"?: AnalysisCommand;
  "migrationFingerprint": string;
  "name": string;
  "origin": string;
  "rootDirectory": string;
  "run"?: AnalysisCommand;
  "staticOutputDirectory": string;
};

export type AnalysisEvidence = {
  "code": string;
  "detail"?: string;
  "field"?: string;
  "path"?: string;
};

export type AnalysisFinding = {
  "code": string;
  "field"?: string;
  "message": string;
  "path"?: string;
  "severity": string;
};

export type AnalysisHealthProbe = {
  "confidence"?: string;
  "evidence": AnalysisEvidence[];
  "method"?: string;
  "origin"?: string;
  "path"?: string;
  "present": boolean;
  "provenance"?: string;
};

export type AnalysisPackageManager = {
  "confidence"?: string;
  "evidence": AnalysisEvidence[];
  "lockfile"?: string;
  "name"?: string;
  "origin"?: string;
  "present": boolean;
  "provenance"?: string;
  "version"?: string;
};

export type AnalysisValue = {
  "confidence"?: string;
  "evidence": AnalysisEvidence[];
  "origin"?: string;
  "present": boolean;
  "provenance"?: string;
  "value"?: string;
};

export type Application = {
  "createdAt": string;
  "description": string;
  "id": string;
  "machineName": string;
  "name": string;
  "slug": string;
  "source": SourceSummary;
  "status": string;
};

export type ApplicationAutoDeployStatus = {
  "activeJobId"?: string;
  "activeSha": string;
  "applicationId": string;
  "enabled": boolean;
  "lastSuccessfulDeployedSha": string;
  "latestResolvedSha": string;
  "nextRetryAt"?: string;
  "pauseCode"?: string;
  "pausedSha": string;
  "retryAttempt": number;
  "revision": number;
  "source": SourceSummary;
  "sourceScopeActive": boolean;
  "state": string;
  "updatedAt": string;
};

export type ApplicationConfiguration = {
  "deploymentPlanRevisionId"?: string;
  "deploymentPlanRevisionNumber"?: number;
  "entries": ConfigurationEntry[];
  "formatVersion"?: number;
  "revisionId"?: string;
  "revisionNumber": number;
  "updatedAt"?: string;
};

export type ApplicationList = {
  "items": Application[];
};

export type ApproveDeploymentPlanMigrationRequest = {
  "expectedApprovalRevision": number;
  "revisionId": string;
  "revisionNumber": number;
};

export type BootstrapRequest = {
  "passphrase": string;
  "token": string;
  "username": string;
};

export type BootstrapStatus = {
  "bootstrapRequired": boolean;
};

export type CSRFResponse = {
  "csrfToken": string;
};

export type Capabilities = {
  "composeRuntime": boolean;
  "fakeRuntime": boolean;
  "generatedRuntime": boolean;
  "githubConnections": boolean;
};

export type ConfigurationEntry = {
  "key": string;
  "phase"?: string;
  "sensitive": boolean;
  "targetComponent"?: string;
  "value"?: string;
};

export type ConfigurationValueInput = {
  "key": string;
  "value": string;
};

export type ConnectedGitHubRepository = {
  "accountLogin": string;
  "archived": boolean;
  "connectionId": string;
  "defaultBranch": string;
  "disabled": boolean;
  "id": number;
  "installationId": number;
  "name": string;
  "owner": string;
  "private": boolean;
};

export type ConnectedGitHubRepositoryPage = {
  "items": ConnectedGitHubRepository[];
  "page": number;
  "perPage": number;
  "totalCount": number;
  "truncated": boolean;
};

export type CreateApplicationRequest = {
  "description"?: string;
  "githubSource"?: GitHubSource;
  "machineId"?: string;
  "name": string;
  "setup"?: DeploymentSetupInput;
  "sourcePath"?: string;
};

export type DefaultSourceConnection = {
  "configured": boolean;
  "connection"?: SourceConnection;
};

export type DeployApplicationRequest = {
  "expectedConfigurationRevisionId"?: string;
  "expectedConfigurationRevisionNumber"?: number;
  "expectedPlanRevisionId"?: string;
  "expectedPlanRevisionNumber"?: number;
};

export type DeployReleaseRequest = {
  "configurationMode": string;
};

export type Deployment = {
  "actualConfigurationRevisionId"?: string;
  "actualConfigurationRevisionNumber": number;
  "appId": string;
  "configurationMode": string;
  "deploymentPlanRevisionId"?: string;
  "deploymentPlanRevisionNumber": number;
  "diagnosticCode"?: string;
  "failureSummary"?: string;
  "findings": PolicyFinding[];
  "finishedAt"?: string;
  "id": string;
  "jobId"?: string;
  "machineId"?: string;
  "releaseId"?: string;
  "runtimeStrategy": string;
  "startedAt"?: string;
  "status": string;
};

export type DeploymentList = {
  "items": Deployment[];
};

export type DeploymentPlanCandidate = {
  "advancedInputs": AnalysisAdvancedInput[];
  "components": AnalysisComponent[];
  "configPath": string;
  "digest": string;
  "evidence": AnalysisEvidence[];
  "findings": AnalysisFinding[];
  "id": string;
  "install"?: AnalysisCommand;
  "kind": string;
  "missingFields": string[];
  "nodeVersion": AnalysisValue;
  "origin": string;
  "packageManager": AnalysisPackageManager;
  "rootDirectory": string;
  "status": string;
};

export type DeploymentPlanComponent = {
  "buildCommand": string;
  "healthProbe": string;
  "installBehavior": string;
  "installDirectory": string;
  "internalPort": number;
  "name": string;
  "nodeVersion": string;
  "packageManager": string;
  "role": string;
  "rootDirectory": string;
  "runCommand": string;
};

export type DeploymentPlanComponentInput = {
  "buildCommand": string;
  "componentId": string;
  "healthProbe": string;
  "internalPort": number;
  "nodeVersion": string;
  "runCommand": string;
};

export type DeploymentPlanDetector = {
  "name": string;
  "sourceStructuralFingerprint": string;
  "version": string;
};

export type DeploymentPlanFieldProvenance = {
  "confidence": number;
  "evidence": string[];
  "field": string;
  "origin": string;
};

export type DeploymentPlanMigration = {
  "approvalStatus"?: string;
  "approvedAt"?: string;
  "approvedBy"?: string;
  "command"?: string;
  "componentName"?: string;
  "environmentKeys"?: string[];
  "evidenceDigest"?: string;
  "present": boolean;
  "rootDirectory"?: string;
};

export type DeploymentPlanRevision = {
  "acceptedAt"?: string;
  "acceptedBy"?: string;
  "canonicalDigest": string;
  "components": DeploymentPlanComponent[];
  "detector": DeploymentPlanDetector;
  "fieldProvenance": DeploymentPlanFieldProvenance[];
  "migration": DeploymentPlanMigration;
  "revisionId"?: string;
  "revisionNumber": number;
  "setup"?: DeploymentSetupInput;
  "source": DeploymentPlanSource;
  "state": string;
  "strategy": string;
};

export type DeploymentPlanSource = {
  "provider": string;
  "repositoryId": number;
  "resolvedDigest": string;
};

export type DeploymentSetupComponentInput = {
  "buildCommand": string;
  "healthProbe": string;
  "id": string;
  "installCommand": string;
  "internalPort": number;
  "nodeVersion": string;
  "outputDirectory": string;
  "packageManager": string;
  "rootDirectory": string;
  "startCommand": string;
  "technology": string;
};

export type DeploymentSetupInput = {
  "components": DeploymentSetupComponentInput[];
  "migrationCommand"?: string;
};

export type DetectedService = {
  "buildContext"?: string;
  "image"?: string;
  "name": string;
};

export type Diagnostics = {
  "architecture": string;
  "caddyManaged": boolean;
  "clientAvailable": boolean;
  "composeAvailable": boolean;
  "composeDetail": string;
  "composeVersion": string;
  "daemonRunning": boolean;
  "dockerDetail": string;
  "dockerVersion": string;
  "engineReady": boolean;
  "os": string;
  "resources": HostResources;
  "startupLimitation": string;
};

export type DoctorCheck = {
  "detail": string;
  "name": string;
  "ok": boolean;
};

export type DoctorResponse = {
  "checks": DoctorCheck[];
  "startupLimitation": string;
};

export type GitHubBranch = {
  "name": string;
  "protected": boolean;
  "sha": string;
};

export type GitHubBranchPage = {
  "items": GitHubBranch[];
  "page": number;
  "perPage": number;
};

export type GitHubConnectionAuthorization = {
  "authorizationId": string;
  "connectionId": string;
  "expiresAt": string;
  "installUrl": string;
  "pollIntervalSeconds": number;
  "userCode": string;
  "verificationUri": string;
};

export type GitHubConnectionAuthorizationStatus = {
  "authorizationId": string;
  "connection": SourceConnection;
  "nextPollAt"?: string;
  "status": string;
};

export type GitHubDeviceAuthorization = {
  "connectionId": string;
  "expiresAt": string;
  "installUrl": string;
  "pollIntervalSeconds": number;
  "userCode": string;
  "verificationUri": string;
};

export type GitHubInstallation = {
  "accountLogin": string;
  "accountType": string;
  "cachedAt": string;
  "id": number;
  "repositorySelection": string;
  "suspendedAt"?: string;
  "targetType": string;
};

export type GitHubInstallationPage = {
  "items": GitHubInstallation[];
  "page": number;
  "perPage": number;
  "totalCount": number;
};

export type GitHubRepository = {
  "archived": boolean;
  "defaultBranch": string;
  "disabled": boolean;
  "id": number;
  "name": string;
  "owner": string;
  "private": boolean;
};

export type GitHubRepositoryPage = {
  "items": GitHubRepository[];
  "page": number;
  "perPage": number;
  "totalCount": number;
};

export type GitHubSource = {
  "branch": string;
  "composePath"?: string;
  "connectionId": string;
  "installationId": number;
  "repositoryId": number;
};

export type GrantRuntimeApprovalRequest = {
  "fingerprint": string;
};

export type HostResources = {
  "diskAvailableBytes": number;
  "diskTotalBytes": number;
  "memoryAvailableBytes": number;
  "memoryTotalBytes": number;
};

export type InspectRequest = {
  "githubSource"?: GitHubSource;
  "setup"?: DeploymentSetupInput;
  "sourcePath"?: string;
};

export type InspectResponse = {
  "analysis": SourceAnalysis;
  "composeCandidates": string[];
  "findings": SourceFinding[];
  "resolvedSha"?: string;
  "services": DetectedService[];
  "source": SourceSummary;
};

export type Job = {
  "attempt": number;
  "checkpoint": string;
  "createdAt": string;
  "errorCode"?: string;
  "errorDetail"?: string;
  "id": string;
  "pauseDisposition"?: string;
  "phase": string;
  "progress": number;
  "requestedBy"?: string;
  "resourceId": string;
  "resourceType": string;
  "status": string;
  "type": string;
  "updatedAt": string;
};

export type JobEvent = {
  "attempt": number;
  "code": string;
  "id": number;
  "jobId": string;
  "level": string;
  "message": string;
  "phase": string;
  "sequence": number;
  "timestamp": string;
};

export type JobEventList = {
  "items": JobEvent[];
};

export type JobList = {
  "items": Job[];
};

export type JobMutationResponse = {
  "created": boolean;
  "job": Job;
};

export type JobResponse = {
  "job": Job;
};

export type LoginRequest = {
  "passphrase": string;
  "username": string;
};

export type Machine = {
  "architecture": string;
  "composeVersion": string;
  "dockerVersion": string;
  "hostname": string;
  "id": string;
  "name": string;
  "os": string;
  "resources": Record<string, unknown>;
  "status": string;
};

export type MachineList = {
  "items": Machine[];
};

export type MeResponse = {
  "user": User;
};

export type PolicyFinding = {
  "capability": string;
  "createdAt": string;
  "deploymentId": string;
  "disposition": string;
  "fingerprint": string;
  "id": string;
  "policyVersion": string;
  "scope": string;
};

export type Problem = {
  "code": string;
  "detail": string;
  "errors"?: Record<string, string>;
  "requestId": string;
  "status": number;
  "title": string;
  "type": string;
};

export type RelayBindingStatus = {
  "bindingId": string;
  "state": string;
  "updatedAt": string;
};

export type RelayBindingSummary = {
  "bindingId": string;
  "connectionId": string;
  "installationId": number;
  "repositoryId": number;
  "state": string;
  "updatedAt": string;
};

export type RelayEnrollmentStart = {
  "authorizationUrl": string;
  "enrollmentId": string;
  "expiresAt": string;
  "status": string;
};

export type RelayEnrollmentStatus = {
  "bindingId"?: string;
  "completedAt"?: string;
  "createdAt": string;
  "enrollmentId": string;
  "expiresAt": string;
  "status": string;
  "updatedAt": string;
};

export type RelayKeyRotationStatus = {
  "expiresAt": string;
  "rotationId": string;
  "state": string;
};

export type RelayKeyRotationSummary = {
  "expiresAt"?: string;
  "inProgress": boolean;
  "state"?: string;
  "updatedAt"?: string;
};

export type RelayStatus = {
  "activeLeases": number;
  "availability": string;
  "diagnosticsUnavailable": boolean;
  "expiredLeases": number;
  "keyRotation": RelayKeyRotationSummary;
  "observerDropped": number;
  "oldestPendingAgeSeconds": number;
  "outcome": string;
  "paused": boolean;
  "pendingCommands": number;
  "readModelAvailable": boolean;
  "removableBindings": RelayBindingSummary[];
  "state": string;
};

export type Release = {
  "appId": string;
  "archiveSha256"?: string;
  "composePath"?: string;
  "configurationRevisionId"?: string;
  "configurationRevisionNumber": number;
  "createdAt": string;
  "deploymentPlanRevisionId"?: string;
  "deploymentPlanRevisionNumber": number;
  "id": string;
  "repositoryId"?: number;
  "repositoryName"?: string;
  "repositoryOwner"?: string;
  "resolvedSha"?: string;
  "runtimeStrategy": string;
  "sourceBranch"?: string;
  "sourceCommitSha"?: string;
  "sourceProvider": string;
  "trackedRef"?: string;
  "workspaceState"?: string;
};

export type ReleaseList = {
  "items": Release[];
};

export type ReplaceApplicationConfigurationRequest = {
  "expectedRevisionNumber": number;
  "remove": string[];
  "secrets": SecretConfigurationValueInput[];
  "variables": ConfigurationValueInput[];
};

export type ReplaceScopedApplicationConfigurationRequest = {
  "entries": ScopedConfigurationValueInput[];
  "expectedRevisionNumber": number;
  "planRevisionId": string;
  "planRevisionNumber": number;
  "publicBuildDisclosureAcknowledged": boolean;
  "remove": ScopedConfigurationKey[];
};

export type ResumeApplicationAutoDeployRequest = {
  "expectedRevision": number;
};

export type RuntimeApproval = {
  "appId": string;
  "capability": string;
  "fingerprint": string;
  "grantedAt": string;
  "grantedBy": string;
  "id": string;
  "policyVersion": string;
  "revokedAt"?: string;
  "revokedBy"?: string;
  "scope": string;
};

export type RuntimeApprovalList = {
  "items": RuntimeApproval[];
};

export type RuntimeApprovalMutationResponse = {
  "approval": RuntimeApproval;
  "created": boolean;
};

export type RuntimeApprovalResponse = {
  "approval": RuntimeApproval;
};

export type ScopedConfigurationKey = {
  "key": string;
  "phase": string;
  "targetComponent": string;
};

export type ScopedConfigurationValueInput = {
  "key": string;
  "phase": string;
  "preserveStoredSecret"?: boolean;
  "sensitive": boolean;
  "targetComponent": string;
  "value": string;
};

export type SecretConfigurationValueInput = {
  "key": string;
  "value": string;
};

export type Service = {
  "id": string;
  "kind": string;
  "name": string;
  "port"?: number;
  "status": string;
};

export type ServiceList = {
  "items": Service[];
};

export type SessionResponse = {
  "csrfToken": string;
  "user": User;
};

export type SourceAnalysis = {
  "candidates": DeploymentPlanCandidate[];
  "findings": AnalysisFinding[];
  "resolvedDigest": string;
  "schemaVersion": string;
  "source": SourceSummary;
  "structuralFingerprint": string;
};

export type SourceConnection = {
  "accessExpiresAt"?: string;
  "connectedAt"?: string;
  "createdAt": string;
  "credentialGeneration": number;
  "disconnectedAt"?: string;
  "id": string;
  "installUrl"?: string;
  "lastErrorCode"?: string;
  "nextPollAt"?: string;
  "pendingExpiresAt"?: string;
  "provider": string;
  "providerLogin"?: string;
  "providerUserId"?: string;
  "refreshExpiresAt"?: string;
  "status": string;
  "updatedAt": string;
};

export type SourceConnectionList = {
  "items": SourceConnection[];
};

export type SourceFinding = {
  "code": string;
  "message": string;
  "path"?: string;
};

export type SourceSummary = {
  "composePath"?: string;
  "connectionId"?: string;
  "installationId"?: number;
  "path"?: string;
  "repositoryId"?: number;
  "repositoryName"?: string;
  "repositoryOwner"?: string;
  "resolvedSha"?: string;
  "trackedBranch"?: string;
  "trackedRef"?: string;
  "type": string;
};

export type StartRelayEnrollmentRequest = {
  "connectionId": string;
  "installationId": number;
  "repositoryId": number;
};

export type SystemStatus = {
  "capabilities": Capabilities;
  "daemon": string;
  "diagnostics": Diagnostics;
};

export type UpdateApplicationAutoDeployRequest = {
  "enabled": boolean;
  "expectedRevision": number;
};

export type User = {
  "id": string;
  "role": string;
  "username": string;
};
