...
function PaginationControls({ label, page, onPageChange, hasNext }: { label: string; page: number; onPageChange: (page: number) => void; hasNext: boolean }) {
...
export function SourceWizard({ onCancel, onCreated }: { onCancel: () => void; onCreated: (id: string) => void }) {
  ...
  const [fieldErrors, setFieldErrors] = useState<{ name?: string; localPath?: string }>({});
  ...
  const save = () => {
    setFormError("");
    const errors: { name?: string; localPath?: string } = {};
    ...
    <form onSubmit={(event) => { event.preventDefault(); save(); }} noValidate>
      ...
      {kind === "local" ? <section className="source-panel" aria-labelledby="local-source-title">
        ...
        {capability.isLoading ? <div className="callout info" role="status">Checking GitHub connection capability…</div> : capability.isError ? <div className="callout danger" role="alert"><strong>Could not check GitHub capability</strong><span>{safeMessage(capability.error, "The controller status could not be loaded.")}</span><button type="button" className="button small" onClick={() => void capability.refetch()}>Retry capability check</button></div> : !githubEnabled ? <div className="callout warning"><strong>GitHub connections are unavailable</strong><span>This controller has not enabled the GitHub App client configuration.</span></div> : <>
          ...
          <div className="wizard-status">{deviceAuthorization ? "Waiting for GitHub authorization." : selectedConnectionId ? `Connection status: ${selectedStatus.replaceAll("_", " ")}.` : "Choose an existing connection or start a new one."}</div>
          {deviceAuthorization && <div className="callout info device-authorization" role="status" aria-live="polite" aria-atomic="true">
          ...
          {selectedConnectionId && !isConnected && !deviceAuthorization && <div className="callout warning"><strong>Connection needs attention</strong><span>{selectedStatus === "access_lost" ? "GitHub access was lost. Start a new connection before browsing repositories." : `This connection is ${selectedStatus.replaceAll("_", " ")}. Start a new connection or choose another connection.`}</span></div>}
          {isConnected && <div className="source-selects">
            ...
            <PaginationControls label="GitHub App installations" page={installationPage} onPageChange={changeInstallationPage} hasNext={(installations.data?.page ?? 0) * (installations.data?.perPage ?? pageSize) < (installations.data?.totalCount ?? 0)} />
            ...
            <PaginationControls label="repositories" page={repositoryPage} onPageChange={changeRepositoryPage} hasNext={(repositories.data?.page ?? 0) * (repositories.data?.perPage ?? pageSize) < (repositories.data?.totalCount ?? 0)} /></>}
            ...
            <PaginationControls label="branches" page={branchPage} onPageChange={changeBranchPage} hasNext={(branches.data?.items.length ?? 0) === (branches.data?.perPage ?? pageSize)} /></>}
          </div>}
...
function SourceSelect({ label, id, value, onChange, loading, error, disabled, placeholder, emptyTitle, emptyMessage, onRetry, items }: { label: string; id: string; value: string; onChange: (value: string) => void; loading: boolean; error: unknown; disabled: boolean; placeholder: string; emptyTitle: string; emptyMessage: string; onRetry: () => void; items: Array<{ value: string; label: string }> }) {
  return <div className="field">
    ...
    <select id={id} value={value} disabled={disabled || Boolean(error)} onChange={(event) => onChange(event.target.value)}><option value="">{loading ? `Loading ${label.toLowerCase()}…` : error ? `${label} unavailable` : placeholder}</option>{items.map((item) => <option key={item.value} value={item.value}>{item.label}</option>)}</select>
    ...
    {!loading && !error && items.length === 0 && <div className="callout info"><strong>{emptyTitle}</strong><span>{emptyMessage}</span></div>}
  </div>;
}

function InspectionSummary({ inspection, discovery = false }: { inspection: InspectResponse | null; discovery?: boolean }) {
  if (!inspection) return null;
  if (inspection.findings.length > 0) return <div className="callout warning"><strong>Source requires changes before it can be saved</strong>{inspection.findings.map((finding) => <span key={`${finding.code}:${finding.path ?? ""}`}>{finding.message}</span>)}</div>;
  if (discovery && inspection.composeCandidates.length === 0) return <div className="callout warning"><strong>No Compose files found</strong><span>Add a Compose file to the tracked branch, then inspect again.</span></div>;
